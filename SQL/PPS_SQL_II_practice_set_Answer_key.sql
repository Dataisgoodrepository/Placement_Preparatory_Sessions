...
############# SQL II: Window Functions and CTE ###################
...
-- List employees who have the same salary as the employee with the highest salary in their respective department.
   ...
   SELECT e.employee_id, e.name, e.department, e.salary
   FROM employees e
   JOIN DeptMaxSalary d ON e.department = d.department AND e.salary = d.max_salary;
...
AS 'new_roll_number' FROM students;


# Q3. Use the aggregate window functions to display the sum of marks in each row within its partition (Subject).

SELECT student_id , subject , name , marks ,
sum(marks) OVER (PARTITION BY subject ORDER BY marks DESC)
AS 'sum_of_marks_in_each_subject' FROM students;
...
AS 'subjectwise_rank' FROM students;
...
SELECT ad_clicks, day as current_day, 
...
SELECT day, no_users, FIRST_VALUE(no_users) OVER() FROM website_stats WHERE website_id = 3 ;
